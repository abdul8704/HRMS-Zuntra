    ...
    if (type === "role") {
      const fetchRoleEmployees = async () => {
        ...
        try {
          ...
          console.error("Error fetching employees by role:", err?.response?.data?.message || err.message);
        } finally {
          setIsLoading(false);
        }
      };
  ...
  useEffect(() => {
    const fetchCourseProgress = async () => {
      ...
      try {
        ...
        console.error("Error fetching course progress:", err?.response?.data?.message || err.message);
      } finally {
        setIsLoading(false);
      }
    };
              ...
              <>
                ...
                {navId === "courses" && (
                  <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {courseProgress.length > 0 ? (
                        courseProgress.map((course) => (
                          <EmployeeCourseProgress key={course.courseId} data={course} />
                        ))
                      ) : (
                    ...
                    {/* Floating Add Button */}
                    <button
                      className="fixed bottom-8 right-8 w-14 h-14 bg-[#BBD3CC] text-[#6c6c6c] rounded-full flex items-center justify-center text-2xl font-bold cursor-pointer hover:scale-110 hover:bg-[#A6C4BA] transition-transform z-[1000]"
                ...
                {roleProfiles.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                    {roleProfiles.map((profile) => (
                      <div
                        key={profile._id}
                        onClick={() => navigate(`/employee/${profile._id}/details/attendance`)}
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-gray-500">
                    <p className="text-sm md:text-base">No employees found for this role.</p>
                  </div>
                )}
              </>
            )}
