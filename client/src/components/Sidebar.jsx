...
export const Sidebar = ({ role = "HR" }) => {
  ...
  const sidebarItems = [
    ...
    // Add user profile item before logout
    {
      role: "EMP",
      label: "My Profile",
      path: `/employee/${user.userid}/details/attendance`,
      icon: `${BASE_URL}/uploads/profilePictures/${user.userid}.png`,
    },
    {
      role: "EMP",
      label: "Logout",
    ...
    userRole === "newUser"
      ...
      : sidebarItems
        ...
        .map((item) => {
          ...
          if (item.label === "My Profile" && user?.userid) {
            return {
              ...item,
              icon: (
                <div
                  className="w-10 h-10 rounded-full overflow-hidden border-2 border-white cursor-pointer my-4"
                >
                  <img
                    src={`${item.icon}`}
                    alt="Profile"
